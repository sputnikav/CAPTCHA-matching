...
def repeat_downloading_and_saving_images(captcha_id, challenge_id, count, method="orb"):
    ...
    if method == "orb":
        ...
        imgs = list(map(lambda img: cv.copyMakeBorder(threshold_image(img), 200, 200, 200, 200, cv.BORDER_CONSTANT, value=0), raw_imgs))
    else:
        output_folder = f"./images_sift_new_{count}"
        imgs = list(map(lambda img: cv.copyMakeBorder(threshold_image(img), 1, 1, 1, 1, cv.BORDER_CONSTANT, value=0), raw_imgs))
    ...
    for i in [1,2,3]:
        save_arr_to_img(
        arr=imgs[i],
        path=os.path.join(output_folder, f"item_{i}.png")
        )
    return
